...
def results_thread_main(strategy):
    ...
    while True:
        try:
            result = strategy._final_q.get(0.0005)
    ...
    def _process_pending_results(self, iterator, one_pass=False, max_passes=None):
        ...
        while True:
            try:
                ...
                #print("TASK RESULT: (%s) %s" % (task_result._host, task_result._result))
            except IndexError:
                task_result = None
            finally:
            ...
            if one_pass or max_passes is not None and cur_pass >= max_passes:
                break
